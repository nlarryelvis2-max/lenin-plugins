...
def base_url(config: dict | None = None) -> str:
    url = str(
        os.environ.get("LENIN_OWNER_PLATFORM_URL")
        or (config or load_config()).get("platform_url")
        or PROD_BASE
    ).rstrip("/")
...
def request(path: str, *, method: str = "GET", body: dict | None = None, token: str = "") -> dict:
    ...
    session_cookie = str(os.environ.get("LENIN_OWNER_SESSION_COOKIE") or "").strip()
    if not credential and not session_cookie:
    ...
    for attempt in range(attempts):
        ...
        try:
            with urllib.request.urlopen(req, timeout=45 if idempotent_mutation else 30) as response:
