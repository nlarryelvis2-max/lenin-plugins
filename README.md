...
фильтруемых списков и карточек компаний, проектов и пользователей: логины,
статусы, состав, проектные grants, наличие пароля и состояние Uplink. Владелец
может создавать компании и самостоятельные project workspaces, назначать
владельцев и участников, выпускать приглашения, читать проектные командные чаты
и писать в них от своего имени или от имени Ленина. Для сообщения сначала
возвращается полный preview с точным автором, адресатом, проектом, командной
видимостью и текстом; публикация выполняется только отдельным вызовом по
одноразовому confirmation token после явного подтверждения владельца.
Обращение к одному участнику — это видимое всей проектной команде
`@упоминание`, а не приватный direct message. Портфельный и проектный дайджесты показывают общие документы,
материалы, обязательства, уточнения, heartbeat/autopilot receipts и статусы
интеграций. Отдельное подтверждаемое поручение запускает обычного Ленина внутри
выбранного проекта с уже существующим Project MCP; стабильный `operation_id`
не даёт повторному HTTP-запросу создать второе сообщение или agent turn.
Дочерний проект остаётся тем же типом project workspace;
parent и раздельное наследование участников/материалов доступны в явных полях,
а не образуют третью сущность. Отдельные команды читают
личный контекст и project-scoped переписку выбранного пользователя с
обязательной причиной в admin audit.

Существующий пароль посмотреть нельзя: платформа хранит только hash. Владелец
может явно подтвердить сброс и получить новый одноразовый временный пароль.
Uplink показывает только агрегаты и состояние клиентов — без raw paths, имён
файлов, текста сессий или download-доступа. Обычный клиентский токен
`project:collaborate` административных инструментов не открывает.
Роль `admin` также не может выпустить или использовать owner MCP token: это
отдельная возможность только глобальной роли `owner`, проверяемой на каждом
запросе.

Пакетное создание пишет временные пароли только в указанный локальный `.tsv`
с правами `0600`. Существующие логины пропускаются и не сбрасываются.
...
python3 -m unittest discover -s owner/scripts -p 'test_*.py' -v
